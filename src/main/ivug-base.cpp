...
static Evas_Object *create_fullview(Evas_Object *win, ug_data *ugd)
{
	...
	/* Create Full view */
	base = create_layout(win, EDJ_PATH"/ivug-base.edj", "ivug_base");
	MSG_IMAGEVIEW_HIGH("set layout. EDJ=%s Group=%s", EDJ_PATH"/ivug-base.edj", "ivug_base");
	if (base == NULL) {
...
bool on_create( void *priv)
{
	...
	elm_theme_extension_add(NULL, EDJ_PATH"/ivug-base.edj");
...
/*	if (mode == UG_MODE_FULLVIEW) {
	...
	ugd->base = create_fullview(conform, ugd);
	MSG_ERROR("111111111111111111111111111111111111111111");
//	ugd->base = create_fullview(win, ugd);
//	elm_win_resize_object_add(win, ugd->base);
	MSG_ERROR("22222222222222222222222222222222");
...
ON_CREATE_ERROR:
	...
	if (ugd->base == NULL) {
		MSG_IMAGEVIEW_HIGH("0000000000000000000000000000000000000000");
